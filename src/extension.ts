...
function updateDecorations(editor: vscode.TextEditor): void {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (match && i > 0) {
            const markLineText = lines[i];

            let markVisualLength = 0;
            for (const ch of markLineText) {
                markVisualLength += ch === '\t' ? tabSize - (markVisualLength % tabSize) : 1;
            }

            const padCount = Math.max(1, rulerColumn - markVisualLength);
            const range = new vscode.Range(i, 0, i, markLineText.length);
            ...
            decorations.push({
                ...
                renderOptions: {
                    before: {
                        contentText: '\u00A0',
                        textDecoration: `none; display: block; width: ${rulerColumn}ch; height: 0; border-bottom: 0.1px ${resolvedStyle} ${resolvedColor}`,
                    },
                },
            });
        }
